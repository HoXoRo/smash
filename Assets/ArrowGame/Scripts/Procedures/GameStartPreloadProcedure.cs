...
public class GameStartPreloadProcedure : ProcedureBase
{
    ...
    private bool m_IsRemoteSyncStep;
    private float m_RemoteSyncFraction;
    private bool m_HasInitGameFrameworkSettings;
    private bool m_HasAppliedSpecStatus;
    protected override void OnEnter(IFsm<IProcedureManager> procedureOwner)
    {
        ...
        CommonHelper.LogEvent(AdjustEventCodeEvent.loading_enter);
    }
    ...
    protected override void OnUpdate(IFsm<IProcedureManager> procedureOwner, float elapseSeconds, float realElapseSeconds)
    {
        ...
        float targetProgress = (loadedProgress + m_RemoteSyncFraction) / totalProgress;
        smoothProgress = Mathf.Lerp(smoothProgress, targetProgress, elapseSeconds * progressSmoothSpeed);

        GF.BuiltinView.SetLoadingProgress(Mathf.Min(smoothProgress, 0.88f));
        // if (!GF.Setting.GetPrivacyPolicyAgree())
        // {
        //     if (!GF.BuiltinView.IsShowPrivacyPolicy())
        //     {
        //         GF.BuiltinView.ShowPrivacyPolicy();
        //     }
        //     return;
        // }
        //预加载完成 切换场景
        ...
        {
            if (!m_HasInitGameFrameworkSettings)
            {
                m_HasInitGameFrameworkSettings = true;
            ...
            // ChangeState<GameStartMainProcedure>(procedureOwner);
            // CommonHelper.LogEvent(AdjustEventCodeEvent.loading_succeeded);
            
            if (GF.AdjustManager.CheckAttributionCallBack())
            {
                preloadAllCompleted = true;
                GF.LogInfo("预加载完成, 进入游戏场景.");
                // procedureOwner.SetData<VarString>(ChangeSceneProcedure.P_SceneName, "Game");
    ...
    private void InitGameFrameworkSettings()
    {
        ...
        // }
        Dictionary<string, ArrowSoundGroupTable> defaultSoundGroupData = new Dictionary<string, ArrowSoundGroupTable>();
        //初始化SoundGroup
        var soundGroupTb = GF.DataTable.GetDataTable<ArrowSoundGroupTable>();
        ...
        //初始化UIGroup
        var uiGroupTb = GF.DataTable.GetDataTable<ArrowUIGroupTable>();
    ...
    private async void PreloadAndInitData()
    {
        ...
        m_HasInitGameFrameworkSettings = false;
        m_HasAppliedSpecStatus = false;
        smoothProgress = 0;
        totalProgress = 0;
        loadedProgress = 0;
        m_RemoteSyncFraction = 0f;
        m_DataTablesCount = -1;
        var appConfig = await AppConfigs.GetInstanceSync();
        RemoteTableManager.EnsureInitialized(appConfig);
        m_IsRemoteSyncStep = RemoteTableManager.IsEnabled();
        totalProgress = appConfig.DataTables.Length + appConfig.Configs.Length + 2;//2是加载多语言和创建框架扩展
        if (m_IsRemoteSyncStep)
        {
            totalProgress++;
        }
        CreateGFExtension();
    ...
    private async void OnLoadGFExtensionSuccess(string assetName, object asset, float duration, object userData)
    {
        ...
        if (null != GameObject.Instantiate(gfExtPfb, Vector3.zero, Quaternion.identity, GF.Base.transform))
        {
            ...
            if (m_IsRemoteSyncStep)
            {
                GF.LogInfo("开始远程配表同步...");
                m_RemoteSyncFraction = 0f;
                await RemoteTableManager.SyncAsync(progress => m_RemoteSyncFraction = progress);
                m_RemoteSyncFraction = 0f;
                loadedProgress++;
            }
    ...
    private void OnLoadConfigSuccess(object sender, GameEventArgs e)
    {
        ...
        if (!m_HasAppliedSpecStatus && GF.Config.HasConfig("SpecStatus"))
        {
            m_HasAppliedSpecStatus = true;
            CommonHelper.ApplySpecStatusUserType();
            AdjustManager.Instance.GetAttribution();
        }
    }
    ...
    private void OnLoadDataTableSuccess(object sender, GameEventArgs e)
    {
        ...
        loadedProgress++;
        m_DataTablesCount--;
        Log.Info($"Load DataTable Success:{args.DataTableAssetName}..{m_DataTablesCount}");
