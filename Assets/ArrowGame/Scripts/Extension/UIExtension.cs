...
public static class UIExtension
{
    ...
    public static void SetSprite(this Image image, string spriteName, bool resize = false, float scale = 1)
    {
        ...
        GF.UI.LoadSprite(spriteName, sp =>
        {
            if (sp != null)
            {
                if (image != null)
                {
                    ...
                    if (scale != 1) image.transform.localScale = new Vector3(scale, scale, 1);
                }
    ...
    public static void LoadSprite(this UIComponent uiCom, string spriteName, GameFrameworkAction<Sprite> onSpriteLoaded)
    {
        ...
        {
            Log.Warning("UIExtension.LoadSprite()失败, asset:{0}, error:{1}", assetName, errorMessage);
            onSpriteLoaded.Invoke(null);
        }));
    }
    /// <summary>
    ...
    public static void ShowToast(this UIComponent ui, string text)
    {
        ...
        uiParams.Set<VarString>(ArrowToastTips.P_Text, text);
        ...
        ui.OpenUIForm(UIViews.ArrowToastTips, uiParams);
    }
    ...
    {
        var uiTb = GF.DataTable.GetDataTable<ArrowUITable>();
    ...
    {
        if (GF.DataTable == null || !GF.DataTable.HasDataTable<ArrowUITable>())
        {
