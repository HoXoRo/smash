...
public static class EntityExtension
{
    ...
    /// </summary>
    /// <param name="eCom"></param>
    /// <param name="pfbName">预制体资源名(相对于Assets/ArrowGame/Prefabs/Entity目录)</param>
    ...
    public static async Task<int> ShowEntityAsync<T>(this EntityComponent entityComponent, string logicName, string groupName, string prefabName, EntityParams entityParams) where T : EntityLogic
    {
        ...
        entityComponent.ShowEntity(entityId, typeof(T), assetName, groupName, 0, entityParams);
        
        System.EventHandler<GameFramework.Event.GameEventArgs> onShowEntitySuccess = null;
        System.EventHandler<GameFramework.Event.GameEventArgs> onShowEntityFailure = null;

        onShowEntitySuccess = (sender, e) =>
        {
            var args = e as ShowEntitySuccessEventArgs;
            if (args != null && args.Entity.Id == entityId)
            {
        ...
        onShowEntityFailure = (sender, e) =>
        {
            var args = e as ShowEntityFailureEventArgs;
            if (args != null && args.EntityId == entityId)
            {
                GF.Event.Unsubscribe(ShowEntitySuccessEventArgs.EventId, onShowEntitySuccess);
                GF.Event.Unsubscribe(ShowEntityFailureEventArgs.EventId, onShowEntityFailure);
                tcs.SetResult(false);
    ...
    public static EntityParams SetParentTransform(this EntityParams entityParams, Transform parent)
    {
        entityParams.ParentTransform = parent;
        return entityParams;
    }
}
