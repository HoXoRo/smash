...
public class UpdateResourcesProcedure : ProcedureBase
{
    ...
    void CheckVersion()
    {
        if (GFBuiltin.Resource.ResourceMode == GameFramework.Resource.ResourceMode.Updatable || GFBuiltin.Resource.ResourceMode == GameFramework.Resource.ResourceMode.UpdatableWhilePlaying)
        {
            ...
            string verFileUrl = UtilityBuiltin.AssetsPath.GetCombinePath(ConstBuiltin.CheckVersionUrl, Application.version, GetPlatformPath(), ConstBuiltin.VersionFile);
    ...
    private void OnWebRequestFailure(object sender, GameEventArgs e)
    {
        ...
        GFBuiltin.LogError($"请求版本信息失败: {arg.ErrorMessage},断网是否允许进游戏:{ConstBuiltin.NoNetworkAllow}");
        if (ConstBuiltin.NoNetworkAllow)
        {
            GFBuiltin.LogError($"断网允许进游戏:{ConstBuiltin.NoNetworkAllow},使用内置版本进游戏：{GFBuiltin.Resource.InternalResourceVersion}");
            GFBuiltin.Resource.CheckVersionList(GFBuiltin.Resource.InternalResourceVersion);
        }
    }
