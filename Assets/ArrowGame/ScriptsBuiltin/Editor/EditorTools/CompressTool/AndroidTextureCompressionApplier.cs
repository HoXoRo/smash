...
{
    public static class AndroidTextureCompressionApplier
    {
        private const string ArrowGameRoot = "Assets/ArrowGame";

        [MenuItem("ArrowGame/Android压缩/应用到 ArrowGame 全部资源", false, 110)]
        public static void ApplyToAllArrowGame()
        {
            if (!EditorUtility.DisplayDialog(
                    "Android 压缩配置",
                    "将为 ArrowGame 下所有贴图与 SpriteAtlas 应用 Android 默认压缩配置（ASTC）。\n是否继续？",
                    "继续",
                    "取消"))
                return;
            ...
            Debug.Log($"[Android压缩] 完成：贴图 {texturePaths.Count}，图集 {atlasPaths.Count}");
        }

        [MenuItem("ArrowGame/Android压缩/应用到选中资源", false, 111)]
        public static void ApplyToSelection()
        {
            ...
            AssetDatabase.SaveAssets();
            AssetDatabase.Refresh();
            Debug.Log($"[Android压缩] 选中资源处理完成：贴图 {texturePaths.Count}，图集 {atlasPaths.Count}");
        }
            ...
            try
            {
                for (int i = 0; i < assetPaths.Count; i++)
                {
                    ...
                    if (EditorUtility.DisplayCancelableProgressBar(
                            $"Android 贴图压缩 ({i + 1}/{assetPaths.Count})",
                            path,
        ...
        public static void ApplyAtlases(IList<string> assetPaths)
        {
            ...
            try
            {
                for (int i = 0; i < assetPaths.Count; i++)
                {
                    string path = assetPaths[i];
                    if (EditorUtility.DisplayCancelableProgressBar(
                            $"Android 图集压缩 ({i + 1}/{assetPaths.Count})",
                            path,
                            (i + 1f) / assetPaths.Count))
                        break;
        ...
        public static bool ApplyTextureAtPath(string assetPath)
        {
            ...
            var category = AndroidTextureCompressionSettings.GetCategory(assetPath);
        ...
        public static bool ApplyAtlasAtPath(string assetPath)
        {
            ...
            Debug.LogWarning($"[Android压缩] 当前 Unity 版本不支持 SpriteAtlasImporter: {assetPath}");
            return false;
#else
            ...
            var category = AndroidTextureCompressionSettings.GetCategory(assetPath);
            var androidSettings = AndroidTextureCompressionSettings.CreateAndroidPlatformSettings(category, hasAlpha: true);
