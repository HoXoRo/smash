...
namespace UGF.EditorTools
{
    ...
    public class CompressTexturePanel : CompressToolSubPanel
    {
        ...
        private void ApplyIosPanelDefaults()
        {
            IosTextureCompressionSettings.ApplyCompressTexturePanelDefaults(
                compressSettings,
                compressPlatformSettings);

            fallbackTexFormat = IosTextureCompressionSettings.RgbaFormat;
            noAlphaTexFormat = IosTextureCompressionSettings.RgbFormat;

            overrideTextureType = true;
            overrideSpriteMode = true;
            overrideMeshType = true;
            overrideAlphaIsTransparency = true;
            overrideReadable = true;
            overrideGenerateMipMaps = true;
            overrideFilterMode = true;
            overrideMaxSize = true;
            overrideFormat = true;
            overrideCompresserQuality = true;
        }

        private void ApplyAndroidPanelDefaults()
        {
            AndroidTextureCompressionSettings.ApplyCompressTexturePanelDefaults(
                compressSettings,
                compressPlatformSettings);

            fallbackTexFormat = AndroidTextureCompressionSettings.RgbaFormat;
            noAlphaTexFormat = AndroidTextureCompressionSettings.RgbFormat;
        ...
        TextureImporterFormat noAlphaTexFormat = TextureImporterFormat.ASTC_8x8;
#else
        ...
        {
            [BuildTarget.iOS] = new[] { TextureImporterFormat.ASTC_6x6, TextureImporterFormat.ASTC_8x8 },
            [BuildTarget.Android] = new[] { AndroidTextureCompressionSettings.RgbaFormat, AndroidTextureCompressionSettings.RgbFormat },
        ...
        Dictionary<BuildTarget, TextureImporterFormat> texNoAlphaFormatPlatforms = new Dictionary<BuildTarget, TextureImporterFormat>
        {
            ...
            [BuildTarget.Android] = AndroidTextureCompressionSettings.RgbFormat,
        ...
        {
            [BuildTarget.iOS] = IosTextureCompressionSettings.DefaultMaxTextureSize,
            [BuildTarget.Android] = AndroidTextureCompressionSettings.DefaultMaxTextureSize,
        ...
        public override void OnEnter()
        {
            ...
            {
#if UNITY_ANDROID
                format = AndroidTextureCompressionSettings.RgbaFormat,
                maxTextureSize = AndroidTextureCompressionSettings.DefaultMaxTextureSize,
                compressionQuality = AndroidTextureCompressionSettings.CompressionQuality,
                name = AndroidTextureCompressionSettings.AndroidPlatformName,
                overridden = true,
                textureCompression = TextureImporterCompression.Compressed
#elif UNITY_IOS
                format = IosTextureCompressionSettings.RgbaFormat,
                maxTextureSize = IosTextureCompressionSettings.DefaultMaxTextureSize,
                compressionQuality = IosTextureCompressionSettings.CompressionQuality,
                name = IosTextureCompressionSettings.IosPlatformName,
                overridden = true,
                textureCompression = TextureImporterCompression.Compressed
#else
            ...
            if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.iOS)
                ApplyIosPanelDefaults();
            else if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android)
                ApplyAndroidPanelDefaults();
        }
        ...
        public override void DrawBottomButtonsPanel()
        {
            EditorGUILayout.BeginHorizontal("box");
            {
                ...
                if (GUILayout.Button("iOS默认配置", GUILayout.Height(30), GUILayout.MaxWidth(100)))
                {
                    ApplyIosPanelDefaults();
                }
                if (GUILayout.Button("Android默认配置", GUILayout.Height(30), GUILayout.MaxWidth(110)))
                {
                    ApplyAndroidPanelDefaults();
                }
                if (GUILayout.Button("应用iOS到列表", GUILayout.Height(30), GUILayout.MaxWidth(110)))
                {
                    var imgList = GetSelectedAssets();
                    IosTextureCompressionApplier.ApplyTextures(imgList);
                    AssetDatabase.Refresh();
                }
                if (GUILayout.Button("应用Android到列表", GUILayout.Height(30), GUILayout.MaxWidth(120)))
                {
                    var imgList = GetSelectedAssets();
                    AndroidTextureCompressionApplier.ApplyTextures(imgList);
                    AssetDatabase.Refresh();
                }
        ...
        private void StartCompressUnityAssetMode()
        {
            ...
            for (int i = 0; i < totalCount; i++)
            {
                ...
                if (overrideFormat)
                {
                    TextureImporterFormat destFormat;
                    if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.iOS)
                    {
                        var category = IosTextureCompressionSettings.GetCategory(assetName);
                        ...
                        destFormat = iosSettings.format;
                        if (overrideMaxSize)
                            texPlatformSetting.maxTextureSize = iosSettings.maxTextureSize;
                    }
                    else if (EditorUserBuildSettings.activeBuildTarget == BuildTarget.Android)
                    {
                        var category = AndroidTextureCompressionSettings.GetCategory(assetName);
                        bool hasAlpha = texImporter.DoesSourceTextureHaveAlpha();
                        var androidSettings = AndroidTextureCompressionSettings.CreateAndroidPlatformSettings(category, hasAlpha);
                        destFormat = androidSettings.format;
                        if (overrideMaxSize)
                            texPlatformSetting.maxTextureSize = androidSettings.maxTextureSize;
                    }
                    else
                    {
                        destFormat = texImporter.DoesSourceTextureHaveAlpha() ? compressPlatformSettings.format : noAlphaTexFormat;
                    }

                    hasChange = texPlatformSetting.format != destFormat || hasChange;
                    if (texPlatformSetting.format != destFormat)
                    {
                        texPlatformSetting.overridden = true;
