...
{
	public class TutorialWormhole : TutorialBase
	{
		...
		[SerializeField]
		private Transform itemIconTransform;
		...
		public override void Play(TutorialManager manager, Action<int, int> onStageComplete)
		{
			...
			{
				tutorialProgress.tutorialWormholeDone = true;
				SaveService.Save();
			}
		...
		{
			if (levelData == null || (SaveService.Data?.TutorialProgress?.tutorialWormholeDone ?? false))
			{
				return false;
			}

			return levelData.GetDistinctObjectTypes()?.Contains(ObjectType.Wormhole) ?? false;
		}
		...
		private IEnumerator TutorialRoutine(TutorialManager manager)
		{
			...
			{
				itemIconTransform.localScale = Vector3.zero;
			}
			...
			if (itemIconTransform != null)
			{
				_activeIntroSequence.Insert(iconStartTime, itemIconTransform.DOScale(Vector3.one, iconScaleDuration).SetEase(iconScaleCurve));
			}
