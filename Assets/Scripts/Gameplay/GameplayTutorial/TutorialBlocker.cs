...
{
	public class TutorialBlocker : TutorialBase
	{
		...
		public override void Play(TutorialManager manager, Action<int, int> onStageComplete)
		{
			...
			{
				tutorialProgress.tutorialBlockerDone = true;
				SaveService.Save();
			}
		...
		{
			if (levelData == null || (SaveService.Data?.TutorialProgress?.tutorialBlockerDone ?? false))
			{
				return false;
			}
			...
			for (int i = 0; i < levelData.stages.Count; i++)
			{
				...
				if (stage?.blockers != null && stage.blockers.Count > 0)
				{
					return true;
				}
			}
