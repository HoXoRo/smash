...
{
	public class TutorialVerticalTable : TutorialBase
	{
		...
		public override void Play(TutorialManager manager, Action<int, int> onStageComplete)
		{
			...
			{
				tutorialProgress.tutorialVerticalTableDone = true;
				SaveService.Save();
			}
		...
		{
			if (levelData == null || (SaveService.Data?.TutorialProgress?.tutorialVerticalTableDone ?? false))
			{
				return false;
			}
