...
{
	public class TutorialRotatingTable : TutorialBase
	{
		...
		public override void Play(TutorialManager manager, Action<int, int> onStageComplete)
		{
			...
			{
				tutorialProgress.tutorialRotatingTableDone = true;
				SaveService.Save();
			}
		...
		{
			if (levelData == null || (SaveService.Data?.TutorialProgress?.tutorialRotatingTableDone ?? false))
			{
				return false;
			}
