...
namespace SRDebugger.Editor
{
    ...
    static class SRInternalEditorUtil
    {
        ...
        public static string GetRootPath()
        {
            ...
            {
                var path = AssetDatabase.GUIDToAssetPath(potentialAsset).Replace('\\', '/');

                if (path.EndsWith(TestPath, StringComparison.Ordinal))
                {
                    var rootPath = path.Substring(0, path.Length - TestPath.Length);
                    return rootPath;
                }
            }
